...
class game{
	...
	updateScore(){
		this.score += 1 + infoBonus*10;
		(debug || true)?console.log("Score :: infoBonus",this.score,infoBonus):null;
		infoBonus = 0;
		this.displayScore();		
	}
	...
	displayScore(){
		...
		push();
		textSize(20);
		text("Score :" + this.score + " Highscore: " + highscore,10,25);
		pop();
	...
	gameOver(cleanup){
		...
		translate(width/2,height/2);
		textSize(50);
		text((!newHighScore)?"Your Score: " + this.score :"New Highscore :" + highscore,0,0);
		pop();
	...
	collision(other){
		...
		if(d <= this.r + other.r){
			...
			if(d <= this.r + other.informationRadius){
				this.learning = 3;
				infoBonus++;
				(debug || true)?console.log("Learning increased by", infoBonus):null;
			}
			this.alert=false;
		}
